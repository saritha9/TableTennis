...
$('#save').click(function(){
		...
		socket.emit('updateAttributes', myid, ballSpeed, maxScore, paddleSpeed);
...
socket.on('updatePaddle', function(position){
	...
	console.log("moving paddle B: " + position);
	
	$("#paddleB").css("top", position);
...
console.log("paused");
	if ($.browser.mozilla) {
		...
		alert("Player B paused the game. \n");
...
function gameLoop() {
	...
	if ((pingpong.scoreA == maxScore && pingpong.scoreA != 0) || (pingpong.scoreB == maxScore && pingpong.scoreB != 0)) {
		...
		socket.emit('updateScore', myid, pingpong.scoreA, pingpong.scoreB);
		socket.emit('endgame', myid, 1);
...
function moveBall() {
	...
	socket.emit('updateBall', myid, ball.x, ball.y);
